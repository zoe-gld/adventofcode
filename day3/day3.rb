...
path_2.split(',').each do |move|
  case move[0]
  when 'R'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << {x: wire_2.last[:x] + 1, y: wire_2.last[:y]} }
  when 'U'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << {x:wire_2.last[:x], y:wire_2.last[:y] + 1} }
  when 'L'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << {x:wire_2.last[:x] - 1, y: wire_2.last[:y]} }
  when 'D'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << {x: wire_2.last[:x], y: wire_2.last[:y] - 1} }
