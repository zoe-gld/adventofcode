...
path_1.split(',').each do |move|
  case move[0]
  when 'R'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_1 << [wire_1.last[0] + 1, wire_1.last[1]] }
  when 'U'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_1 << [wire_1.last[0], wire_1.last[1] + 1] }
  when 'L'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_1 << [wire_1.last[0] - 1, wire_1.last[1]] }
  when 'D'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_1 << [wire_1.last[0], wire_1.last[1] - 1] }
...
path_2.split(',').each do |move|
  case move[0]
  when 'R'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << [wire_2.last[0] + 1, wire_2.last[1]] }
  when 'U'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << [wire_2.last[0], wire_2.last[1] + 1] }
  when 'L'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << [wire_2.last[0] - 1, wire_2.last[1]] }
  when 'D'
    (1..move.match(/\d+/)[0].to_i).each { |i| wire_2 << [wire_2.last[0], wire_2.last[1] - 1] }
